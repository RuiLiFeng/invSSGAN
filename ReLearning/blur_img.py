...
import logging
from training.train_vae_nod_nos_wider import assgin_net
...
def training_loop(config: Config):
    ...
    with strategy.scope():
        ...
        Generator = resnet_biggan_ssgan.Generator(image_shape=[128, 128, 3], embed_y=False,
                                                  ...
                                                  spectral_norm=True)
        ResNet = ImagenetModel(resnet_size=50, num_classes=1000, name='ResNet')
        ...
        def train_step(image, label):
            ...
            with tf.control_dependencies([add_global] + update_ops):
                Dense_opt = Dense_solver.minimize(loss,
                                                  var_list=ResNet.trainable_variables)
        ...
        with tf.init_scope():
            ...
            saver_resnet = tf.train.Saver(ResNet.trainable_variables, restore_sequentially=True)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for iteration in range(config.total_step):
                ...
                if iteration % config.save_per_steps == 0:
                    saver_resnet.save(sess, save_path=config.model_dir + '/resnet.ckpt',
