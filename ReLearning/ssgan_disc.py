...
def training_loop(config: Config):
    ...
    with strategy.scope():
        ...
        Disccriminator = resnet_biggan.Discriminator(spectral_norm=True, project_y=False)
        ...
        def train_step(image, label):
            _, _, w = Disccriminator(image, None, True)
            w = Dense(w)
        ...
        def eval_step(image, label):
            _, _, w = Disccriminator(image, None, is_training=True)
            w = Dense(w)
