...
from network import abstract_arch
from network import arch_ops as ops
from network import resnet_ops
...
class Discriminator(abstract_arch.AbstractDiscriminator):
  ...
  def apply(self, x, y, is_training):
    ...
    with tf.variable_scope("discriminator_rotation", reuse=tf.AUTO_REUSE):
        rotation_scores = ops.linear(tf.reshape(h, (tf.shape(x)[0], -1)),
                                     4,
                                     scope="score_classify",
                                     use_sn=self._spectral_norm)
    return out, out_logit, rotation_scores
