...
def compute_loss(train_step, data, strategy):
    losses = strategy.experimental_run_v2(train_step, (data,))
    mean_losses = []
    for loss in losses:
        mean_losses.append(strategy.reduce(tf.distribute.ReduceOp.MEAN, loss, axis=None))
    return mean_losses
    ...
    with strategy.scope():
        ...
        def train_step(image):
            ...
            with tf.control_dependencies([add_global] + update_ops):
                E_opt = E_solver.minimize(e_loss, var_list=Encoder.trainable_variables + Ebn.trainable_variables)
        ...
        with tf.init_scope():
            ...
            fixed_w = Ebn(fixed_w, is_training=True)
        ...
        with tf.init_scope():
            ...
            saver_ebn = tf.train.Saver(Ebn.trainable_variables, restore_sequentially=True)
            ...
            for iteration in range(config.total_step):
                ...
                if iteration % config.print_loss_per_steps == 0:
                    ...
                    print("step %d, e_loss %f, r_loss %f, s_loss %f, t_loss %f "
                          ...
                          (iteration, e_loss_, r_loss_, s_loss_, 0.0,
                ...
                if iteration % config.save_per_steps == 0:
                    ...
                    saver_ebn.save(sess, save_path=config.model_dir + '/ebn.ckpt',
                                   global_step=iteration, write_meta_graph=False)
...
def triplet_loss(w_, w, margin):
    assert w_.shape.ndims == w.shape.ndims
    if w.shape.ndims == 4:
        w_ = tf.reshape(w_, [w_.shape[0].value, -1])
        w = tf.reshape(w, [w.shape[0].value, -1])
    w_1 = tf.tile(tf.expand_dims(w_, 1), [1, w_.shape[0].value, 1])
    w1 = tf.tile(tf.expand_dims(w, 0), [w.shape[0].value, 1, 1])

    pairwise_dis = tf.reduce_mean(tf.square(w_1 - w1), axis=2)
