...
def compute_loss(train_step, data, strategy):
    loss1, loss2, loss3, loss4 = strategy.experimental_run_v2(train_step, (data,))
    ...
    mean_losses3 = strategy.reduce(tf.distribute.ReduceOp.MEAN, loss3, axis=None)
    mean_losses4 = strategy.reduce(tf.distribute.ReduceOp.MEAN, loss4, axis=None)
    return mean_losses1, mean_losses2, mean_losses3, mean_losses4
    ...
    with strategy.scope():
        ...
        def train_step(image, training_G):
            ...
            with tf.control_dependencies([add_global] + update_ops):
                Embed_opt = Embed_solver.minimize(final_loss, var_list=D_embed.trainable_variables)
        ...
        final_loss, d_loss, r_loss, s_loss = compute_loss(train_step, dataset.get_next(), strategy)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for iteration in range(config.total_step):
                ...
                if iteration % config.print_loss_per_steps == 0:
                    timer.update()
                    print("step %d, final_loss %f, d_loss %f, r_loss %f, s_loss %f, "
