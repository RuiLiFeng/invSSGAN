...
def build_np_dataset(root, batch_size, load_in_mem=False, shuffle_buffer_size=10000):
    h5_dset = ILSVRC128(root, load_in_mem)
...
def training_loop(config: Config):
    ...
    dataset = build_np_dataset(root=config.h5root, batch_size=config.batch_size, load_in_mem=config.load_in_mem)
    ...
    with strategy.scope():
        ...
        def train_step(image):
            ...
            with tf.variable_scope('recon_loss'):
                ...
                vgg_loss = config.r_loss_scale * tf.nn.l2_loss(vgg_fake - vgg_real) / (
                            config.batch_size * feature_scale)
